...
describe('Job', function() {
    ...
    describe('move()', function() {
        it('should move the job to another queue', function(done) {
            client.queue('jhyt').put('FooJob', {}, {"jid": "my4jid"}, function() {
                client.queue('jhyt').pop(function(result) {
                    result.move('ngie', function(result2) {
                        client.queues('ngie', function(result) {
                            var q = result;
                            assert.equal(q.name, 'ngie');
