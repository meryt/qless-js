...
queue.prototype.pop = function(count, cb, eb) {
	if (typeof(count) == 'function') {
		...
		this.client._pop.call([this.name], [this.worker, 1, (new Date().getTime() / 1000)], function(r) {
			if (r.length) {
				cb(new job(selfclient, JSON.parse(r)));
			} else {
				cb({});
			}
		}, eb);
	} else {
		this.client._pop.call([this.name], [this.worker, count, (new Date().getTime() / 1000)], function(r) {
...
queue.prototype.peek = function(count, cb, eb) {
	if (typeof(count) == 'function') {
		...
		this.client._peek.call([this.name], [count, (new Date().getTime() / 1000)], function(r) {
			var results = JSON.parse(r);
			for (var index in results) {
				results[index] = new job(selfclient, results[index]);
			}
